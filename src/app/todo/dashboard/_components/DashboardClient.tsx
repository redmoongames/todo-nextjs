...
import { Task, CompletedTask, useTodoService } from '@/shared/todo';
...
export default function DashboardClient({ initialTasks }: DashboardClientProps) {
  ...
  const [isLoading, setIsLoading] = useState<boolean>(!initialTasks);
  const [error, setError] = useState<string | null>(null);
  const [tasks, setTasks] = useState<Task[]>(
    initialTasks ? todoService.getUncompletedTasks(initialTasks) : []
  );
  const [completedTasks, setCompletedTasks] = useState<CompletedTask[]>(
    initialTasks ? todoService.getCompletedTasks(initialTasks) : []
  );
  const hasFetchedRef = useRef<boolean>(!!initialTasks);
